      ...
      <Grid container spacing={4}>
        {products.map((product) => (
          <Grid item xs={12} md={6} lg={4} key={product.id}>
            ...
            <Card 
              elevation={6} 
              sx={{ borderRadius: 3, border: '1px solid #d7ccc8', bgcolor: '#fffde7' }}
            >
              <CardContent>
                <Typography variant="h5" sx={{ mb: 1, fontWeight: 'bold', color: '#5d4037' }}>
                  {product.name}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  Artisan: {product.artisan}
                </Typography>
                <Typography variant="h6" color="primary" sx={{ my: 1, fontWeight: 'bold', color: '#a1887f' }}>
                  {product.price}
                </Typography>
                <Chip 
